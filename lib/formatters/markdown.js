  ...
  return changelogRenderer.render({
    ...
    renderBody(body, times) {
      body = body.replace(issueMatcher, (...args) => {
        ...
        const formattedURL = typeof opts.issueIDURL === 'function' ? opts.issueIDURL(...args) : sFormat(opts.issueIDURL, b1);
    ...
    renderDescription(log) {
      ...
      shortDescription = shortDescription.replace(issueMatcher, (a, b1) => sFormat('[{1}]({0})', sFormat(opts.issueIDURL, b1), a));
